...
(module (registers register? rax rcx rdx rbx rbp rsi rdi r8 r9 r10 r11 r12 
         r13 r14 r15 heap-size stack-size mref mset! reset-machine-state!
         parameter-registers caller-saved-registers frame-pointer-register
          ...
          (do ([i next (fx+ i 1)])
            ...
            (let ([fvi (string->symbol (format "fv~s" i))])
              (eval `(define-syntax ,fvi
                       (identifier-syntax
                         [var (mref ,frame-pointer-register ,(fxsll i align-shift))]
                         [(set! var val) (mset! ,frame-pointer-register ,(fxsll i align-shift) val)])))
