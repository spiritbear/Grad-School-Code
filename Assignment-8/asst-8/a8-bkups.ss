    ...
    (define select-relop 
		...
		(define select-mref
			(lambda (lhs base offset)
				(cond
					[(or (and (integer? base) (integer? offset))
							 (and (ur? base) (integer? offset))
							 (and (ur? base) (ur? offset))) `((set! ,lhs (mref ,base ,offset)))]
					[(and (integer? base) (ur? offset)) `((set! ,lhs (mref ,offset ,base)))]
					[(or (and (frame-var? base) (or (integer? offset) (ur? offset)))
							 (and (label? base) (or (integer? offset) (ur? offset))))
								(let ((u (new-u)))
									`((set! ,u ,base) (set! ,lhs (mref ,u ,offset))))]
					[(or (and (frame-var? base) (or (frame-var? offset) (label? offset)))
							 (and (label? base) (or (frame-var? offset) (label? offset))))
												(let ((u1 (new-u)) (u2 (new-u)))
		...
		(define select-mset
			(lambda (base offset value)
				(cond
					...
					[(and (integer? base) (ur? offset)) `((mset! ,offset ,base ,value))]
					[(or (and (frame-var? base) (or (integer? offset) (ur? offset)))
							 (and (label? base) (or (integer? offset) (ur? offset))))
								(let ((u (new-u)))
									`((set! ,u ,base) (mset! ,u ,offset ,value)))]
					[(or (and (frame-var? base) (or (frame-var? offset) (label? offset)))
							 (and (label? base) (or (frame-var? offset) (label? offset))))
												(let ((u1 (new-u)) (u2 (new-u)))
													`((set! ,u1 ,base) (set! ,u2 ,offset) (mset! ,base ,offset ,value)))]
					[else (let ((u (new-u)))
												`((set! ,u ,offset) (mset! ,base ,offset ,value)))])))
    (define Effect 
			(lambda (ef)
				(match ef
	        [(nop) '(nop)]
	        ...
	        [(if ,[Pred -> test] ,[Effect -> conseq] ,[Effect -> altern]) `(if ,test ,conseq ,altern)]
					[(set! ,lhs (mref ,base ,offset))
						(cond
							[(ur? lhs) (make-begin (select-mref lhs base offset))]
							[(or (label? lhs) (frame-var? lhs)) 
								(let ((u (new-u)))
									 `(begin (set! ,u ,lhs) ,(select-mref u base offset) ...))])]
	        ...
	        [(set! ,lhs ,rhs) (select-move lhs rhs)]
					[(mset! ,base ,offset ,value)
						(cond 
							...
							[(or (frame-var? value) (label? value))
								(let ((u (new-u)))
									`(begin (set! ,u ,value) ,(select-mset base offset u) ...))])]
